...
test('R6.4.1 is the single final output release identity',()=>{
  assert.equal(OI.RELEASE_META.version,'R6.4.1');
  assert.equal(OI.RELEASE_META.ruleYear,2026);
  assert.match(OI.releaseLabel(),/Calculation build R6\.4\.1/);
});
...
test('R6.4.1 interpretation warnings remain visible without expanding the engine',()=>{
  const source=read('output-integrity.js');
