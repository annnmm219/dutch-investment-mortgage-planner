(function(root,factory){
  ...
  const api=factory(SC,OI);
  if(typeof module==='object'&&module.exports)module.exports=api;
  if(root){root.NextEuroCore=api;root.NextEuro=api;}
})(typeof globalThis!=='undefined'?globalThis:this,function(SC,OI){
'use strict';
if(!SC)throw new Error('ScenarioCore is required by NextEuroCore');
if(!OI)throw new Error('OutputIntegrity is required by NextEuroCore');
...
'use strict';
const FC=window.FinanceCore,NE=window.NextEuroCore,OI=window.OutputIntegrity,II=window.InputIntegrity,$=id=>document.getElementById(id),clamp=FC.clamp;
if(!OI||!II)throw new Error('OutputIntegrity and InputIntegrity must load before next-euro.js');
...
function baseConfig(){if(window.__DIMP_ACTIVE_SCENARIO_CONFIG)return JSON.parse(JSON.stringify(window.__DIMP_ACTIVE_SCENARIO_CONFIG));const hra=window.LogicIntegrityUI?.mortgageTaxContext?.()||{hraRemainingMonths:360,qualifyingInterestFraction:1};return{startYear:clamp(num('startYear',2026),2020,2100),startMonth:clamp(num('startMonth',1),1,12),startPortfolio:Math.max(0,num('startPortfolio')),mortgageType:selectedMortType(),mortgage:mainMortgage(),tax:{enabled:$('mortTaxEnabled')?.checked!==false,deductionRate:FC.deductionRate2026({mode:$('deductionMode')?.value||'auto',manualRatePct:num('manualDeduction',37.56),grossIncome:num('grossIncome')}),wozValue:Math.max(0,num('wozValue')),hraRemainingMonths:hra.hraRemainingMonths,qualifyingInterestFraction:hra.qualifyingInterestFraction},box3:box3(),vveMonthly:Math.max(0,num('scenarioVveNew',250)),maintenanceAnnual:Math.max(0,num('scenarioMaintenanceNew',1500)),ownerTaxesAnnual:Math.max(0,num('scenarioOwnerTaxesNew')),insuranceAnnual:Math.max(0,num('scenarioInsuranceNew')),groundLeaseAnnual:Math.max(0,num('scenarioGroundLeaseNew'))}}
const REQUIRED_CONTEXT_IDS=['startYear','startPortfolio','mortBalance','mortRate','mortYears','housePrice','ownSavings','purchaseCosts','purchaseRate','purchaseYears','grossIncome','wozValue','manualDeduction','hraRemainingYears','hraRemainingMonths','qualifyingBox1DebtPct','currentTaxRate','currentAllowance','currentNotional','futureStart','futureTaxRate','futureExempt','futureLossThreshold','box3Savings','box3Debt','box3SavingsReturn','box3DebtInterest','box3DebtMonthlyRepayment','currentSavingsNotional','currentDebtNotional','currentDebtThreshold','firstJan1Portfolio','firstJan1Savings','firstJan1Debt','scenarioVveNew','scenarioMaintenanceNew','scenarioOwnerTaxesNew','scenarioInsuranceNew','scenarioGroundLeaseNew'];
function render(){const report=II.validateControls([...card.querySelectorAll('input')]);let canonical;if(!report.valid||!window.__DIMP_ACTIVE_SCENARIO_CONFIG){canonical=OI.canonicalNextEuroResult({main:{valid:false,reason:!report.valid?II.summary(report):'Complete and review the Scenario inputs first.'}});}else{const amount=num('nextEuroAmount'),years=num('nextEuroYears'),ret=num('nextEuroReturn'),x=NE.analyseNextEuro(baseConfig(),{amount,years,assumedReturnPct:ret});canonical=OI.canonicalNextEuroResult(x);}const model=OI.nextEuroOutputModel(canonical);window.__DIMP_CANONICAL_NEXT_EURO=canonical;$('nextEuroBreakEven').textContent=model.breakEvenValue;$('nextEuroBreakEvenSub').textContent=model.breakEvenSub;$('nextEuroWinner').textContent=model.choiceValue;$('nextEuroWinnerSub').textContent=model.choiceSub;$('nextEuroDifference').textContent=model.differenceValue;const body=$('nextEuroQuickBody');body.innerHTML='';canonical.quick.forEach(r=>{const tr=document.createElement('tr'),outcome=r.outcome==='invest'?'Invest':r.outcome==='repay'?'Repay mortgage':r.outcome==='tie'?'Modeled tie':'Unavailable';tr.innerHTML=`<td>${fmt(r.amount)}</td><td>${r.breakEvenStatus==='found'?pct(r.breakEvenReturnPct):r.breakEvenStatus==='outside-range'?'Outside tested range':'Unavailable'}</td><td>${outcome}</td><td>${r.valid?fmt(r.absoluteDifference):'Unavailable'}</td>`;body.appendChild(tr)})}
